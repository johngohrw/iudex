...
2. **Read path** = `iudex status --json`, parsed into a `Workspace` view model. The GUI is the *filter*; the CLI returns the whole truth.
...
```jsonc
{
  ...
  "tickets": [
    {
      "id": "t1", "state": "active",
      "deps": ["t2"], "blockedBy": [], "blocks": ["t5"],
      "qaRejects": 0, "ready": true,
      "hasWorktree": true, "worktree": "/abs/path/.iudex/worktrees/t1"
    }
  ]
}
```
...
**Accepted-as-is (deliberate):** agent spawning has zero guards (a stray double-click fires real billable `claude`); synthesized status can mislead (it's a heuristic); dead sessions accumulate (manual clear-finished); binary/huge files render blank in the diff.
...
1. **Dashboard auto-activate toggle** — the lone opt-in automation: auto-run `activate` for queued + ready + under-capacity tickets on each doorbell tick. (Spec'd, not built.)
2. **Dependency-DAG toggle** inside Tickets (a mode, not a nav slot).
3. **Recent-projects launcher** + smoother multi-window scoping (the tmux pool is currently global; per-workspace `@iudex_root` tagging + filtering is the planned fix).
4. **Multi-workspace tmux scoping** — stamp `@iudex_root` on spawn, filter `list_sessions`/`clear_finished` by root.
...
**The boundary, restated:** the GUI may *act* (write via `iudex`) and *observe* (read via `status --json`, watch `events.jsonl`, supervise tmux), but the CLI alone *decides*. Every authoritative state transition is a real `iudex` command.
